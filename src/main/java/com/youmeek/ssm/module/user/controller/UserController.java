...
//    private UserService service;
    ...
    public ResponseEntity<?> regist(@RequestBody SysUser user) {
//        service.save(user);
//        Result<String> result = ResultUtil.buildSuccessResult("注册成功");
     ...
     * @return
     */
    @ApiOperation(value = "根据pk查找用户", notes = "返回用户实体对象", response = SysUser.class, position = 2)
    @ResponseBody
...
//        Result<Userser> result = ResultUtil.buildSuccessResult(service.findByPk(userPk));
        return new ResponseEntity<String>("", HttpStatus.OK);
    }
